...
class CharBuffer
{
  public:
	...
	// fill the buffer with a given char
	void fill(const char t_filler = ' ')
	{
		for (auto i = 0; i < BufferSize; i++)
		{
			m_buffer[i] = t_filler;
		}
		m_bufferIndex = BufferSize - 1;
	}

	// fill the remaining space in the buffer by a given char 
	void fillRemaining(const char t_filler = ' ')
	{
		while (print(t_filler))
			;
	}
